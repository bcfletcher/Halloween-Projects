...
def sensorThread():
    ...
    while True:
        ...
        if debug2: print("count=",c," sensor/pin_14=",sensorPin.value()," Mode Pin_11=",modePinIn.value())
        lightsleep(50)
        c = c + 1
        if sensorPin.value() == modePinIn.value():
            if debug: print("Waiting for animatronics to run, sensor value=",sensorPin.value()," Mode Pin=",modePinIn.value())
            runAnimatronic=True
            while runAnimatronic == True:
                pass
            set_brightness(green, 100)
            if debug: print("Waiting for sensor detection, sensor value=",sensorPin.value()," Mode Pin=",modePinIn.value())
        else:
            pass
...
def main():
    ...
    if debug: print("Mode Pin=",modePinIn.value())
